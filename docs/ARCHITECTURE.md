...
## 8. Future agent integration boundary

Hermes, if introduced, is a separate orchestration and user-facing agent layer; it does not
replace this application. This service remains the owner of ingestion, retained knowledge,
preferences, provider routing, and its administrative controls.

Any future integration is an application-facing, versioned, domain-oriented API rather than a
Hermes-specific control surface. Candidate namespaces are `/api/v1/briefings/...`,
`/api/v1/knowledge/...`, `/api/v1/preferences/...`, and `/api/v1/system/...`. They must expose
only deliberately selected, least-privilege views or commands. M18 implements the first read-only
subset under `/api/v1/briefings/...` and `/api/v1/knowledge/search`; it is independently token
authenticated, bounded, audited with metadata only, and disabled by default. The contract is in
`docs/AGENT_API.md`.

Hermes must not receive direct database access, Gmail OAuth material, OpenRouter credentials, or
shell access through this service. If coding automation is later needed, it belongs in a separate
coding-worker/Codex CLI trust zone with its own authorization, filesystem, and audit policy; it is
not part of the AI Assistant deployment boundary.
