...
class TokenCipher:
    """Small reversible-at-rest boundary using an environment-supplied secret key."""
    ...
    def protect(self, value: str) -> str:
        encrypted = bytes(
            byte ^ self._key[index % len(self._key)] for index, byte in enumerate(value.encode())
        )
    ...
    def reveal(self, value: str) -> str:
        ...
        return bytes(
            byte ^ self._key[index % len(self._key)] for index, byte in enumerate(encrypted)
        ).decode()
